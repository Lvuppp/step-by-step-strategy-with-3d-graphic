...
void Building::addObject(Object3D *object)
{
    if (!object)
        return;
...
Object3D *Building::GetObject(quint32 index)
{
...
void Building::CalculateTBN(QVector<VertexData> &data)
{
...
const QVector3D &Building::GetLocation() const
{
    return objects.last()->GetLocation();
}

const int &Building::GetBlockPosition() const
{
    return positionOnBlock;
}

void Building::SetBlockPosition(const int& blockNumber)
{
    positionOnBlock = blockNumber;
}


void Building::draw(QOpenGLShaderProgram *program, QOpenGLFunctions *functions, bool usingTextures)
{
    for (qsizetype i = 0; i < objects.size(); ++i)
        objects[i]->draw(program, functions, usingTextures);
}

void Building::Rotate(const QQuaternion &r)
{
    for (qsizetype i = 0; i < objects.size(); ++i)
        objects[i]->Rotate(r);
}

void Building::Translate(const QVector3D &t)
{
    for (qsizetype i = 0; i < objects.size(); ++i)
        objects[i]->Translate(std::move(t));
}

void Building::Scale(const float &s)
{
    for (qsizetype i = 0; i < objects.size(); ++i)
        objects[i]->Scale(s);
}

void Building::SetGlobalTransform(const QMatrix4x4 &q)
{
    for (qsizetype i = 0; i < objects.size(); ++i)
